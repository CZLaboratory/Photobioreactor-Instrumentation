...
s = ttk.Style()
s.theme_create( "greeny", parent="alt", settings={
    "TNotebook": {"configure": {"tabmargins": [2, 5, 2, 0], "background": colorg } },
    "TNotebook.Tab": {
        "configure": {"padding": [5, 1], "background": colorg },
        "map":       {"background": [("selected", colorg)],
...
def updateVal():
    ...
    if valueLED.get():
        try:
            if len(valueLED.get())==0: ##null entry?
                ...
                pwv=round(int(float(valueLED.get()))*40.95)  ##Round var and scale to 0-4095
            pwmWhite(pwv)  ##set white led intensity percent
        except ValueError:
            messagebox.showerror(title='Not a number', message='Please only numbers')
            valueLED.set("0")
    ...
    if valueGRE.get():
        try:
            if len(valueGRE.get())==0: ##null entry?
                ...
                pgv=round(int(float(valueGRE.get()))*40.95)  ##Round var and scale to 0-4095
    ...
    if valueRED.get():
        try:
            if len(valueRED.get())==0: ##null entry?
                ...
                prv=round(int(float(valueRED.get()))*40.95)  ##Round var and scale to 0-4095
    ...
    if valueBLU.get():
        try:
            if len(valueBLU.get())==0: ##null entry?
                ...
                pbv=round(int(float(valueBLU.get()))*40.95)  ##Round var and scale to 0-4095
    ...
    if valuePmp.get():
        try:
            if len(valuePmp.get())==0: ##null entry?
                ...
                ppv=round(int(float(valuePmp.get()))*40.95)  ##Round var and scale to 0-4095
    ...
    datatosave={valuePH.get()+','+valueOD.get()+','+valueTMP.get()+','+valueLVL.get()+','+valueCO2.get()+
                ','+valueLED.get()+','+valueRED.get()+','+valueGRE.get()+','+valueBLU.get()+','+ valuePmp.get()}       
    
    ##data info message
    if flg==0:
        datatosave={'PH value,DO value,Temperature value,Level value,CO2 value'+
...
txtOD=Label(frameView, text="Dissolved Oxygen (mg/L)", font=("Arial", 12), bg=colorg)
...
txtCO2=Label(frameView, text="Concentration CO2 (ppm)", font=("Arial", 12), bg=colorg)
...
txtLED=Label(frameView, text="White light intensity (%)", font=("Arial", 12), bg=colorg)
...
txtRED=Label(frameView, text="Red light intensity (%)", font=("Arial", 12), bg=colorg)
...
txtGRE=Label(frameView, text="Green light intensity (%)", font=("Arial", 12), bg=colorg)
...
txtBLU=Label(frameView, text="Blue light intensity (%)", font=("Arial", 12), bg=colorg)
...
txtPMP=Label(frameView, text="Air pump power (%)", font=("Arial", 12), bg=colorg)
...
txtLT=Label(frameLight, text="Select type of light", font=("Arial", 12, "bold"), bg=colorg)
txtLT.grid(row=0, column=1, pady=3, padx=3, columnspan=8)

whiteLight=Radiobutton(frameLight, text="White light", font=("Arial", 12), variable=lightOption, value=1, command=types, bg=colorg)
whiteLight.grid(row=1, column=1, pady=3, padx=3, columnspan=4)

rgbLight=Radiobutton(frameLight, text="RGB light", font=("Arial", 12), variable=lightOption, value=2, command=types, bg=colorg)
rgbLight.grid(row=1, column=5, pady=3, padx=3, columnspan=4)

#Read value white bright
txtWhInt=Label(frameLight, text="White light intensity (0-100%)", font=("Arial", 12), bg=colorg)
...
WhSlid=Scale(frameLight, from_=0, to=100,length=400, orient='horizontal', variable=valueLED, bg=colorg)
WhSlid.grid(row=3, column=2, pady=3, padx=3, columnspan=6)

txtrgbInt=Label(frameLight, text="Please, put red, green and blue intensity (0-100%)",  font=("Arial", 12), bg=colorg)
...
putRedInt=Entry(frameLight, textvariable=valueRED, width=8, font=("Arial", 12))
...
putGreInt=Entry(frameLight, textvariable=valueGRE, width=8, font=("Arial", 12))
...
putBluInt=Entry(frameLight, textvariable=valueBLU, width=8, font=("Arial", 12))
...
WhSlid= Scale(frameLight, from_=0, to=100,length=400, orient='horizontal', variable=valueRED, bg=colorg)
...
WhSlid = Scale(frameLight, from_=0, to=100,length=400, orient='horizontal', variable=valueGRE, bg=colorg)
...
WhSlid = Scale(frameLight, from_=0, to=100,length=400, orient='horizontal', variable=valueBLU, bg=colorg)
WhSlid.grid(row=8, column=2, pady=3, padx=3, columnspan=6)

#Read value air pump bright
txtPInt=Label(frameLight, text="Air pump power (0-100%)", font=("Arial", 12), bg=colorg)
...
PmSlid = Scale(frameLight, from_=0, to=100,length=400, orient='horizontal', variable=valuePmp, bg=colorg)
PmSlid.grid(row=10, column=2, pady=3, padx=3, columnspan=6)

#-------------------------------------------------------------------------------------------------
#Indicators 3rd tab
txtLg=Label(frameConfiguration, text="Data logger configuration", font=("Arial", 12, "bold"), bg=colorg)
txtLg.grid(row=1, column=1, pady=3, padx=3, columnspan=6)

txtLog=Label(frameConfiguration, text="Save data logger", font=("Arial", 12), bg=colorg)
txtLog.grid(row=2, column=3, pady=3, padx=3, sticky="W")
chkLog=Checkbutton(frameConfiguration, text="Enable", font=("Arial", 12), variable=logActive, onvalue=1, offvalue=0, bg=colorg )
chkLog.grid(row=2, column=4, pady=3, padx=3, sticky="W")

txtFName=Label(frameConfiguration, text="File name", font=("Arial", 12), bg=colorg)
txtFName.grid(row=3, column=3, pady=3, padx=3, sticky="W")
putFName=Entry(frameConfiguration, textvariable=valueFName, width=12, font=("Arial", 12))
putFName.grid(row=3, column=4, pady=3, padx=3, sticky="W")

txtRoute=Label(frameConfiguration, text="File path", font=("Arial", 12), bg=colorg)
txtRoute.grid(row=4, column=1, pady=3, padx=3)
putRoute=Entry(frameConfiguration, textvariable=valueRoute, width=70, font=("Arial", 12))
putRoute.grid(row=4, column=2, pady=3, padx=3, columnspan=4)
btnRoute=Button(frameConfiguration, text="Save", font=("Arial", 12), command=browsepath)
